...
import { signOut as fbSignOut } from '../firebase/auth.js'
import { seedTestCheckins, clearTestCheckins } from '../utils/seedTestData.js'
...
export default function Profile() {
  ...
  function cancelDevPress() {
    clearTimeout(devPressRef.current)
  }
  ...
  function handleSave() {
    updateUser({ ...form, tonight_status: Array.isArray(form.tonight_status) ? form.tonight_status.join(' · ') : form.tonight_status })
    setSaved(true)
  ...
  function handlePhoto(index, e) {
    ...
    reader.onload = ev => {
      const key = `photo${index + 1}_url`
      updateUser({ [key]: ev.target.result })
    }
      ...
      <div className="px-4 py-6 space-y-5 pb-10">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
          <h1 className="text-2xl font-black text-white select-none"
            ...
            onTouchStart={startDevPress} onTouchEnd={cancelDevPress}>👤 {t(lang, 'profile')}</h1>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
          ...
          <div>
            ...
            <p className="text-sm text-white/40">{user?.age}{user?.gender ? ` · ${user.gender === 'male' ? t(lang, 'male') : user.gender === 'female' ? t(lang, 'female') : t(lang, 'other')}` : ''}</p>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[0, 1, 2].map(i => (
              <div key={i} onClick={() => fileRefs[i].current?.click()}
                className="aspect-square rounded-xl overflow-hidden border border-dashed border-white/20 flex items-center justify-center cursor-pointer hover:border-[hsl(290,100%,65%,0.5)] transition-colors"
        ...
        ].map(({ key, label, type, prefix, maxLength }) => (
          <motion.div key={key} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
            ...
            {type === 'textarea' ? (
              <div className="relative">
                <textarea value={form[key]} onChange={e => up(key, e.target.value)} rows={3} maxLength={maxLength}
                ...
                {maxLength && <span className="absolute bottom-2 left-3 text-[10px] text-white/20">{form[key]?.length || 0}/{maxLength}</span>}
              </div>
            ) : (
              <div className="relative">
                {prefix && <span className="absolute left-4 top-1/2 -translate-y-1/2 text-white/40 text-sm">{prefix}</span>}
                <input type={type} value={form[key]} onChange={e => up(key, e.target.value)} maxLength={maxLength}
        ...
        <div>
          ...
          <div className="space-y-1.5">
            {t(lang, 'statusOptions').map(s => {
              const selected = Array.isArray(form.tonight_status) ? form.tonight_status.includes(s) : form.tonight_status === s
              return (
        ...
        {/* Dev tools — always visible for test account, long-press header for others */}
        {(showDevTools || user?.is_test_account) && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            ...
            {seedStatus && <p className="text-xs text-center font-mono" style={{ color: seedStatus.startsWith('✅') ? '#4ade80' : seedStatus.startsWith('❌') ? '#f87171' : '#fde68a' }}>{seedStatus}</p>}
          </motion.div>
        )}
