...
import { likePerson, superLikePerson, listenFeed } from '../firebase/db'
import { canSuperLike, getSuperLikesRemaining, useSuperLike } from '../utils/storage'
...
export default function Feed() {
  ...
  const { notifyMatch, requestPermission } = usePushNotifications()
  ...
  // Request notification permission after user starts browsing
  useEffect(() => {
    if (!checkin) return
  ...
  function handlePass(person) {
    ...
    import('../utils/storage').then(m => m.passPerson(person.id))
  }
